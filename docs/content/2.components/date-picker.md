...
<template>
  <VCalendarDatePicker v-if="date && (date instanceof Date)" v-model="date" :columns="2" v-bind="{ ...attrs, ...$attrs }" />
  <VCalendarDatePicker v-else v-model.range="date" v-bind="{ ...attrs, ...$attrs }" />
</template>
